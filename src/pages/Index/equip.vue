...
<script>
import data from "../../data/wzry-equip";
const equips = Object.values(data);
export default {
  data() {
    return {
      list: [],
    ...
    onLoad() {
      setTimeout(() => {
        ...
        for (let i = 0, ind = this.list.length; i < 10; i++, ind++) {
          if (ind < equips.length) {
            // console.log(ind, equips.length);
            this.list.push(equips[ind]);
          }
        }
        // this.list = equips;
        this.loading = false;

        if (this.list.length === equips.length) {
          this.finished = true;
        }
      }, 0);
    },
